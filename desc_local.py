...
def compute_local_descriptors(mesh, sample_size, avg_bins):
    """
    ...
    local_features = {
        'A3': list(np.histogram(a_3, avg_bins['A3'], range=hist_ranges['A3'])[0]),
        'D1': list(np.histogram(d_1, avg_bins['D1'], range=hist_ranges['D1'])[0]),
        'D2': list(np.histogram(d_2, avg_bins['D2'], range=hist_ranges['D2'])[0]),
        'D3': list(np.histogram(d_3, avg_bins['D3'], range=hist_ranges['D3'])[0]),
        'D4': list(np.histogram(d_4, avg_bins['D4'], range=hist_ranges['D4'])[0])
    }
...
if __name__ == "__main__":
    ...
    if not os.path.exists(average_bins_path):
        avg_bins = compute_all_bins_parallel(preprocessed_shapes_df, sample_size=2000, preprocessed_dataset_path=PREPROCESSED_DATASET_PATH, num_processes=num_processes)
        print(avg_bins)
    else:
